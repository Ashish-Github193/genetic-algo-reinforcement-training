...
function Model(num_neurons, num_layers, input_shape) {
  ...
  this.initialise = () => {
    for (let index = 0; index < this.num_layers; index++) {
      if (index == 0) {
        this.weights.push(
          new Array(this.input_shape)
            .fill(0)
            .map(() =>
              Array.from(
                { length: this.num_neurons[index] },
                () => Math.random() * [-1, 1][rand(1)]
              )
            )
        );
      } else {
        this.weights.push(
          new Array(this.num_neurons[index - 1])
            .fill(0)
            .map(() =>
              Array.from(
                { length: this.num_neurons[index] },
...
function GENETIC_ALGORITHM(
  ...
  this.CROSSOVER_MUTATION_NETWORKS = (fitnesses, continue_saved_model = 0) => {
    if (continue_saved_model == 0) {
      ...
      do {
        parent_2_idx = rand(0, this.elite_networks - 1);
    ...
    for (let idx = 0; idx < this.parent1.weights.length; idx++) {
      shapes.push([
        this.parent1.weights[idx].length,
        this.parent1.weights[idx][0].length,
      ]);
    }
    ...
    child1_genes = genes2
      .slice(0, start)
      .concat(
        genes1.slice(start, start + length).concat(genes2.slice(start + length))
      );
    child2_genes = genes1
      .slice(0, start)
      .concat(
        genes2.slice(start, start + length).concat(genes1.slice(start + length))
      );
    ...
    for (let ch = 0; ch < this.population.length - 2; ch++) {
      ...
      child_gene_cm = gene
        .slice(0, start)
        .concat(random_parent_gene.slice(start, start + length))
        .concat(gene.slice(start + length));
  ...
  this.CREATE_POPULATION = (
    ...
    parent1_weights = null,
    parent2_weights = null
  ) => {
    this.population = [];
    for (let index = 0; index < size; index++) {
      this.population.push(new Model(num_neurons, num_layers, input_shape));
    }

    if (parent1_weights != null && parent2_weights != null) {
      this.parent1.weights = parent1_weights;
      this.parent2.weights = parent2_weights;
      this.CROSSOVER_MUTATION_NETWORKS(
        Array.from({ length: 2 }, () => 0),
        (continue_saved_model = 1)
      );
    }
  };
